...
class WeixinAddonModel extends WeixinModel {
	function reply($dataArr, $keywordArr = array()) {
		...
		if ($dataArr ['Content'] == 'subscribe') {
			...
			$map2 ['uid'] = $uid = M ( 'apps_follow' )->where ( $map1 )->getField ( 'uid' );
			$credit ['uid'] = $uid;
			add_credit ( 'unsubscribe', 0, $credit );
			
			// 如果需要用户取消关系时系统自动物理删除该用户，把下面三行开启即可
			// M ( 'apps_follow' )->where ( $map1 )->delete ();
	...
	function scan($dataArr, $keywordArr = array(), $config = array()) {
		...
		if ($qr ['addon'] == 'UserCenter') { // 设置用户分组
			...
			$map1 ['token'] = get_token ();
			$followId = M ( 'apps_follow' )->where ( $map1 )->getField ( 'uid' );
			...
			$articles [0] = array (
					...
					'PicUrl' => SITE_URL . "/Addons/Card/View/Public/cover_pic.png",
			...
			$map1 ['openid'] = $dataArr ['FromUserName'];
			$map1 ['token'] = get_token ();
			$map2 ['uid'] = D ( 'apps_follow' )->where ( $map1 )->getField ( 'uid' );
			D ( 'apps_follow' )->where ( $map1 )->setField ( 'uid', $qr ['extra_int'] );
			M ( 'user' )->where ( $map2 )->delete ();
			M ( 'credit_data' )->where ( $map2 )->delete ();
			...
			if ($info) {
				...
				$replace = array (
						...
						addons_url ( 'WeiSite://Wap/index', $param ),
						$param ['token'],
						$param ['openid'] 
				);
